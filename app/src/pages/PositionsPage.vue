<template>
  <q-page padding>
    <div class="row items-center justify-between q-mb-md">
      ...
      <q-btn color="primary" icon="add" label="Новая должность" @click="openDialog()" />
    </div>
    ...
    <q-dialog v-model="dialog.show" persistent>
      <q-card style="min-width: 400px">
        <q-card-section class="row items-center">
          <div class="text-h6">{{ dialog.isEdit ? 'Редактирование' : 'Создание' }} Должности</div>
          <q-space />
        ...
        <q-card-section class="q-gutter-y-md">
          <q-input
            ...
            outlined
            dense
            :rules="[val => !!val || 'Необходимо заполнить']"
          />
          <q-input
...
const columns: QTableColumn[] = [
  ...
  { name: 'name', label: 'Название должности', field: 'name', align: 'left', sortable: true },
...
const loadData = async () => {
  ...
  try {
    ...
    $q.notify({ color: 'negative', message: 'Не удалось загрузить должности' });
  } finally {
    loading.value = false;
  }
};
...
const save = async () => {
  ...
  try {
    if (dialog.isEdit && dialog.activeId) {
      await api.patch(`/positions/${dialog.activeId}`, form);
    } else {
